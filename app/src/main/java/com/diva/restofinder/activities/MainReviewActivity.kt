...
import com.diva.restofinder.view.ItemsActivity
import com.diva.restofinder.view.UploadActivity
import kotlinx.android.synthetic.main.activity_review_main.*
import kotlinx.android.synthetic.main.activity_review_main.btnAddInfo
import kotlinx.android.synthetic.main.activity_review_main.btnViewAll
import org.json.JSONException
import org.json.JSONObject
import java.util.*

class MainReviewActivity : AppCompatActivity(), LocationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchResto.setQueryHint("Food Suggestion")
...
//        rvCollection.setAdapter(mainAdapterHorizontal)
        ...
        mainAdapter?.setOnItemClickCallback(object : OnItemClickCallback {
            override fun onItemMainClicked(modelMain: ModelMain?) {
                val intent = Intent(this@MainReviewActivity, DetailRestoActivity::class.java)
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject) {
                    try {
                        ...
                        e.printStackTrace()
                        Toast.makeText(this@MainReviewActivity, "Loading data!", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onError(anError: ANError) {
                    mProgressBar?.dismiss()
                    Toast.makeText(this@MainReviewActivity, "Cannot connnect internet!", Toast.LENGTH_SHORT).show()
                }
            })
    }
...
//                            //modelMainHorizontal.add(dataApi)
                ...
                override fun onError(anError: ANError) {
                    mProgressBar?.dismiss()
                    Toast.makeText(this@MainReviewActivity, "Cannot Connect internet!", Toast.LENGTH_SHORT).show()
                }
            })
    }
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject) {
                    try {
                        ...
                        e.printStackTrace()
                        Toast.makeText(this@MainReviewActivity, "Uploading data!", Toast.LENGTH_SHORT).show()
                    }
                }
